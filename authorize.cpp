#include "./headerfiles/crypto/rsa/scrsa.h"
#include "./headerfiles/hwinfo/hwinfo.h"
//#include "./headerfiles/crypto/FourQlib/FourQ_64bit_and_portable/FourQ_api.h"
//#include "./headerfiles/crypto/FourQlib/FourQ_64bit_and_portable/FourQ_params.h"
//#include "./headerfiles/crypto/FourQlib/random/random.h"
//#include "./headerfiles/crypto/FourQlib/sha512/sha512.h"
//#include "./headerfiles/crypto/FourQlib/FourQ_64bit_and_portable/tests/test_extras.h"
#include "./headerfiles/crypto/aes/aes.h"
#include <fstream>
#include "authorize.h"
	
bool authorize(std::string filepath,std::string outputfilepath)
{
	std::string rsaPubKeyStr1 = "03";
	std::string rsaPrivateKeyStr1= "";
	std::string rsaN1 = "";
	std::string rsaPubKeyStr2 = "03";
	std::string rsaPrivateKeyStr2= ""; 
	std::string rsaN2 = "";

    std::ifstream file(filepath);
    if (!file) {
        std::cerr << "Error opening file." << std::endl;
		return false;
    }
    std::string hardwareInfoStr((std::istreambuf_iterator<char>(file)),
                        std::istreambuf_iterator<char>());

	scrsa rsa;
	std::string encryptostr = rsa.decrypt_string(rsaPrivateKeyStr1, rsaN1, hardwareInfoStr);
    std::cout << "plain 2:\n" << hexToString(encryptostr) << std::endl << std::endl;
	std::string liencese = rsa.encrypt_string(rsaPubKeyStr2, rsaN2, encryptostr);
	std::cout << "crypt 3:\n" << hexToString(liencese) << std::endl << std::endl;

	std::ofstream outfile(outputfilepath);
	outfile << liencese.c_str(); 

	return true;
}

std::string authorizeString(std::string cipherstring)
{
	std::string rsaPubKeyStr1 = "03";
	std::string rsaPrivateKeyStr1= "";
	std::string rsaN1 = "";
	std::string rsaPubKeyStr2 = "03";
	std::string rsaPrivateKeyStr2= ""; 
	std::string rsaN2 = "";

	scrsa rsa;
	std::string encryptostr = rsa.decrypt_string(rsaPrivateKeyStr1, rsaN1, cipherstring);
    std::cout << "plain 2:\n" << hexToString(encryptostr) << std::endl << std::endl;
	std::string liencese = rsa.encrypt_string(rsaPubKeyStr2, rsaN2, encryptostr);
	std::cout << "crypt 3:\n" << hexToString(liencese) << std::endl << std::endl;

	return liencese;
}